defmodule LivePlaygroundWeb.DemoHelpers do
  ...
  def placeholder_paragraphs(count, random \\ false) do
    ...
    |> shuffle(random)
    |> Enum.slice(0..(count - 1))
    |> Enum.map(fn text -> Phoenix.HTML.raw("<p>#{text}</p>") end)
  end
  ...
  @doc """
  Creates a link element. This is a compatibility wrapper for the deprecated Phoenix.HTML.Link.link/2.

  ## Examples

      link("Click here", to: "/path", class: "underline")
      link("Reset", to: "#", phx_click: "reset", class: "text-blue-500")
  """
  def link(text, opts) do
    ...
    phx_attr = if phx_click, do: " phx-click=\"#{phx_click}\"", else: ""
    escaped_text = Phoenix.HTML.html_escape(text) |> Phoenix.HTML.safe_to_string()

    Phoenix.HTML.raw(~s(<a href="#{to}" class="#{class}"#{phx_attr}>#{escaped_text}</a>))
  end
end
