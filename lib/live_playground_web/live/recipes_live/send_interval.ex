defmodule LivePlaygroundWeb.RecipesLive.SendInterval do
  ...
  defp format_duration(seconds) when seconds > 0 do
    ...
    parts = []
    parts = if hours > 0, do: parts ++ ["#{hours} hour#{if hours != 1, do: "s"}"], else: parts
    parts = if minutes > 0, do: parts ++ ["#{minutes} minute#{if minutes != 1, do: "s"}"], else: parts
    parts = if secs > 0 or parts == [], do: parts ++ ["#{secs} second#{if secs != 1, do: "s"}"], else: parts
